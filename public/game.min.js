...
// lib/game/entities/otherPlayer.js
ig.baked=true;ig.module('game.entities.otherPlayer').requires('impact.entity').defines(function(){EntityOtherPlayer=ig.Entity.extend({size:{x:12,y:24},offset:{x:10,y:4},zIndex:500,gravityFactor:0,dead:false,type:ig.Entity.TYPE.NONE,checkAgainst:ig.Entity.TYPE.NONE,collides:ig.Entity.COLLIDES.NEVER,animSheet:new ig.AnimationSheet('media/player_tileset.png',32,32),init:function(x,y,settings){this.parent(x,y,settings);this.addAnim('idle_down',1,[0]);this.addAnim('walk_down',0.15,[1,0,2,0]);this.addAnim('idle_side',1,[3]);this.addAnim('walk_side',0.1,[5,5,3,4]);this.addAnim('walk_up',0.15,[6,8,7,8]);this.addAnim('idle_up',1,[8]);this.addAnim('idle_down_shank',1,[16]);this.addAnim('walk_down_shank',0.15,[17,16,18,16]);this.addAnim('idle_side_shank',1,[19]);this.addAnim('idle_up_shank',1,[22]);this.addAnim('walk_side_shank',0.15,[19,21]);this.addAnim('walk_up_shank',0.15,[22,24,23,24]);this.addAnim('attack_down_shank',0.15,[25,26,27],true);this.addAnim('attack_side_shank',0.15,[28,29],true);this.addAnim('attack_up_shank',0.15,[30,31],true);this.addAnim('hit',0.1,[48,49]);this.hitTimer=new ig.Timer(0);this.attackTimer=new ig.Timer(0);},die:function(){this.currentAnim=this.anims.die;this.currentAnim.rewind();this.dead=true;},hit:function(){this.hitTimer=new ig.Timer(0.5);},strike:function(){this.currentAnim=this.anims.attack_down_shank;this.currentAnim.rewind();this.attackTimer=new ig.Timer(0.5);},setAnimation:function(){if(this.shank){if(this.attackTimer.delta()<0){if(this.direction==='up'){this.currentAnim=this.anims.attack_up_shank;}else if(this.direction==='down'){this.currentAnim=this.anims.attack_down_shank;}else if(this.direction==='left'){this.currentAnim=this.anims.attack_side_shank;this.flip=true;}else if(this.direction==='right'){this.currentAnim=this.anims.attack_side_shank;this.flip=false;}}else if(this.vel.x>0){this.direction='right';this.currentAnim=this.anims.walk_side_shank;this.flip=false;}else if(this.vel.x<0){this.direction='left';this.currentAnim=this.anims.walk_side_shank;this.flip=true;}else if(this.vel.y>0){this.direction='down';this.currentAnim=this.anims.walk_down_shank;}else if(this.vel.y<0){this.direction='up';this.currentAnim=this.anims.walk_up_shank;}else{if(this.direction==='left'){this.currentAnim=this.anims.idle_side_shank;this.flip=true;}else if(this.direction==='right'){this.currentAnim=this.anims.idle_side_shank;this.flip=false;}else if(this.direction==='up'){this.currentAnim=this.anims.idle_up_shank;}else if(this.direction==='down'){this.currentAnim=this.anims.idle_down_shank;}else{this.currentAnim=this.anims.idle_down_shank;}}}else{if(this.vel.x>0){this.currentAnim=this.anims.walk_side;this.direction='left';this.flip=false;}else if(this.vel.x<0){this.currentAnim=this.anims.walk_side;this.direction='right';this.flip=true;}else if(this.vel.y>0){this.direction='down';this.currentAnim=this.anims.walk_down;}else if(this.vel.y<0){this.direction='up';this.currentAnim=this.anims.walk_up;}else{if(this.direction==='left'){this.currentAnim=this.anims.idle_side;this.flip=true;}else if(this.direction==='right'){this.currentAnim=this.anims.idle_side;this.flip=false;}else if(this.direction==='up'){this.currentAnim=this.anims.idle_up;}else if(this.direction==='down'){this.currentAnim=this.anims.idle_down;}}}},draw:function(){this.parent();ig.game.font.draw(this.nickname,this.pos.x+16-ig.game.screen.x,this.pos.y-ig.game.screen.y-20,ig.Font.ALIGN.CENTER);},update:function(){if(this.attackTimer.delta()<0){this.currentAnim=this.anims.attack_down_shank;}else if(this.dead){this.currentAnim=this.anims.die;}else{this.setAnimation();}
this.currentAnim.flip.x=this.flip;this.parent();}});});

// lib/game/entities/heart.js
ig.baked=true;ig.module('game.entities.heart').requires('impact.entity').defines(function(){EntityHeart=ig.Entity.extend({size:{x:64,y:64},zIndex:1000,gravityFactor:0,heartTime:5,type:ig.Entity.TYPE.NONE,checkAgainst:ig.Entity.TYPE.NONE,collides:ig.Entity.COLLIDES.NEVER,animSheet:new ig.AnimationSheet('media/heart.png',64,64),init:function(x,y,settings){this.parent(x,y,settings);this.addAnim('idle',0.12,[0,1,2,3,4,0,1],true);this.currentAnim=this.anims.idle;this.heartTimer=new ig.Timer(this.heartTime*this.player.pulse);ig.game.sortEntitiesDeferred();},update:function(){if(this.heartTimer.delta()>0){ig.game.sfxHeart.play();this.heartTimer.set(this.heartTime*this.player.pulse);this.currentAnim.rewind();}
this.pos.x=ig.game.screen.x+15;this.pos.y=ig.game.screen.y+15;this.parent();}});});

// lib/game/entities/player.js
ig.baked=true;ig.module('game.entities.player').requires('impact.entity','game.entities.heart').defines(function(){EntityPlayer=ig.Entity.extend({size:{x:12,y:24},offset:{x:10,y:4},zIndex:500,gravityFactor:0,baseSpeed:60,speed:60,flashlight:true,flip:false,shank:false,pulse:1,type:ig.Entity.TYPE.NONE,checkAgainst:ig.Entity.TYPE.NONE,collides:ig.Entity.COLLIDES.NEVER,animSheet:new ig.AnimationSheet('media/player_tileset.png',32,32),init:function(x,y,settings){this.parent(x,y,settings);this.addAnim('idle_down',1,[0]);this.addAnim('walk_down',0.15,[1,0,2,0]);this.addAnim('idle_side',1,[3]);this.addAnim('walk_side',0.1,[5,5,3,4]);this.addAnim('walk_up',0.15,[6,8,7,8]);this.addAnim('idle_up',1,[8]);this.addAnim('idle_down_shank',1,[16]);this.addAnim('walk_down_shank',0.15,[17,16,18,16]);this.addAnim('idle_side_shank',1,[19]);this.addAnim('idle_up_shank',1,[22]);this.addAnim('walk_side_shank',0.15,[19,21]);this.addAnim('walk_up_shank',0.15,[22,24,23,24]);this.addAnim('attack_down_shank',0.15,[25,26,27],true);this.addAnim('attack_side_shank',0.15,[28,29],true);this.addAnim('attack_up_shank',0.15,[30,31],true);this.addAnim('hit',0.1,[48,49]);this.addAnim('die',0.1,[48,49,50,51,52,53,54],true);this.hitTimer=new ig.Timer(0);this.heart=ig.game.spawnEntity(EntityHeart,15,15,{player:this});this.attackTimer=new ig.Timer(0);},hit:function(){this.hitTimer=new ig.Timer(0.5);},die:function(){this.currentAnim=this.anims.die;this.currentAnim.rewind();this.dead=true;this.heart.kill();},strike:function(){if(this.direction==='down'){this.currentAnim=this.anims.attack_down_shank;}else if(this.direction==='left'){this.currentAnim=this.anims.attack_side_shank;this.flip=true;}else if(this.direction==='right'){this.currentAnim=this.anims.attack_side_shank;this.flip=false;}else if(this.direction==='up'){this.currentAnim=this.anims.attack_up_shank;}
this.currentAnim.rewind();this.attackTimer=new ig.Timer(0.5);ig.game.sfxSwordHit.play();},handleMovementTrace:function(res){this.parent(res);if(res.collision.y){miner.setVelY(this.vel.y);}else if(res.collision.x){miner.setVelX(this.vel.x);}},setAnimation:function(){if(this.shank){if(this.attackTimer.delta()<0){if(this.direction==='up'){this.currentAnim=this.anims.attack_up_shank;}else if(this.direction==='down'){this.currentAnim=this.anims.attack_down_shank;}else if(this.direction==='left'){this.currentAnim=this.anims.attack_side_shank;this.flip=true;}else if(this.direction==='right'){this.currentAnim=this.anims.attack_side_shank;this.flip=false;}}else if(this.vel.x>0){this.currentAnim=this.anims.walk_side_shank;this.flip=false;}else if(this.vel.x<0){this.currentAnim=this.anims.walk_side_shank;this.flip=true;}else if(this.vel.y>0){this.currentAnim=this.anims.walk_down_shank;}else if(this.vel.y<0){this.currentAnim=this.anims.walk_up_shank;}else{if(this.direction==='left'){this.currentAnim=this.anims.idle_side_shank;this.flip=true;}else if(this.direction==='right'){this.currentAnim=this.anims.idle_side_shank;this.flip=false;}else if(this.direction==='up'){this.currentAnim=this.anims.idle_up_shank;}else if(this.direction==='down'){this.currentAnim=this.anims.idle_down_shank;}else{this.currentAnim=this.anims.idle_down_shank;}}}else{if(this.vel.x>0){this.currentAnim=this.anims.walk_side;this.flip=false;}else if(this.vel.x<0){this.currentAnim=this.anims.walk_side;this.flip=true;}else if(this.vel.y>0){this.currentAnim=this.anims.walk_down;}else if(this.vel.y<0){this.currentAnim=this.anims.walk_up;}else{if(this.direction==='left'){this.currentAnim=this.anims.idle_side;this.flip=true;}else if(this.direction==='right'){this.currentAnim=this.anims.idle_side;this.flip=false;}else if(this.direction==='up'){this.currentAnim=this.anims.idle_up;}else if(this.direction==='down'){this.currentAnim=this.anims.idle_down;}}}},update:function(){this.speed=this.baseSpeed;if(!this.dead){if(ig.input.pressed('left')){this.vel.x=-this.speed;miner.setVelX(this.vel.x);this.direction='left';}else if(ig.input.pressed('right')){this.vel.x=this.speed;miner.setVelX(this.vel.x);this.direction='right';}else if(ig.input.released('left')){if(ig.input.state('right')){this.vel.x=this.speed;this.direction='right';}else{this.vel.x=0;}
miner.setVelX(this.vel.x);}else if(ig.input.released('right')){if(ig.input.state('left')){this.vel.x=-this.speed;this.direction='left';}else{this.vel.x=0;}
miner.setVelX(this.vel.x);}
if(ig.input.pressed('down')){this.vel.y=this.speed;miner.setVelY(this.vel.y);this.direction='down';}else if(ig.input.pressed('up')){this.vel.y=-this.speed;miner.setVelY(this.vel.y);this.direction='up';}else if(ig.input.released('up')||ig.input.released('down')){this.vel.y=0;miner.setVelY(this.vel.y);}
if(ig.input.pressed('bomb')){miner.blowUp();}
if(ig.input.pressed('drop')){miner.dropItem();}
if(ig.input.pressed('flashlight')){miner.toggleFlashlight();ig.game.sfxClick.play();}
if(ig.input.pressed('interact')){miner.interactWithWorld();}
if(ig.input.pressed('use')){miner.useItem();}}
if(this.pos.x<0){this.pos.x=0;}else if(this.pos.x>499*32){this.pos.x=499*32;}
if(this.pos.y<0){this.pos.y=0;}else if(this.pos.y>499*32){this.pos.y=499*32;}
if(this.hitTimer.delta()<0){this.currentAnim=this.anims.hit;}else if(this.dead){this.currentAnim=this.anims.die;}else{this.setAnimation();}
this.currentAnim.flip.x=this.flip;this.parent();}});});

// lib/game/entities/item.js
ig.baked=true;ig.module('game.entities.item').requires('impact.entity').defines(function(){EntityItem=ig.Entity.extend({size:{x:32,y:32},zIndex:400,gravityFactor:0,type:ig.Entity.TYPE.NONE,checkAgainst:ig.Entity.TYPE.NONE,collides:ig.Entity.COLLIDES.NEVER,animSheet:new ig.AnimationSheet('media/player_tileset.png',32,32),init:function(x,y,settings){this.parent(x,y,settings);this.animSheet=new ig.AnimationSheet('media/'+settings.meta.sprite,32,32);this.addAnim('idle',settings.meta.animations.idle.speed,settings.meta.animations.idle.frames);ig.game.sortEntitiesDeferred();},update:function(){this.currentAnim=this.anims.idle;this.currentAnim.flip.x=this.flip;this.parent();}});});

// lib/game/entities/ai.js
ig.baked=true;ig.module('game.entities.ai').requires('impact.entity').defines(function(){EntityAI=ig.Entity.extend({size:{x:32,y:32},zIndex:500,gravityFactor:0,type:ig.Entity.TYPE.NONE,checkAgainst:ig.Entity.TYPE.NONE,collides:ig.Entity.COLLIDES.NEVER,animSheet:new ig.AnimationSheet('media/enemy_tileset.png',32,32),init:function(x,y,settings){this.parent(x,y,settings);this.addAnim('idle_down',1,[0]);this.addAnim('walk_down',0.15,[0,1,0,2]);this.addAnim('idle_side',1,[3]);this.addAnim('walk_side',0.15,[3,4,5]);this.addAnim('walk_up',0.15,[6,7,6,8]);this.addAnim('idle_up',0.15,[6]);this.addAnim('attack',0.15,[9,10]);this.attackTimer=new ig.Timer(0);},strike:function(){this.currentAnim=this.anims.attack;this.currentAnim.rewind();this.attackTimer=new ig.Timer(0.5);},setAnimation:function(){if(this.vel.x>0){this.currentAnim=this.anims.walk_side;this.flip=false;}else if(this.vel.x<0){this.currentAnim=this.anims.walk_side;this.flip=true;}else if(this.vel.y>0){this.currentAnim=this.anims.walk_down;}else if(this.vel.y<0){this.currentAnim=this.anims.walk_up;}else{this.currentAnim=this.anims.idle_down;}},update:function(){if(this.attackTimer.delta()<0){this.currentAnim=this.anims.attack;}else{this.setAnimation();}
this.currentAnim.flip.x=this.flip;this.parent();}});});

// lib/game/main.js
ig.baked=true;ig.module('game.main').requires('impact.game','impact.font','game.entities.otherPlayer','game.entities.player','game.entities.item','game.entities.ai').defines(function(){MyGame=ig.Game.extend({sfxHeart:new ig.Sound('media/Sound/heartbeat.*'),sfxSwordHit:new ig.Sound('media/Sound/Swordhit.*'),sfxCrafting:new ig.Sound('media/Sound/crafting.*'),sfxConsume:new ig.Sound('media/Sound/consume.*'),sfxClick:new ig.Sound('media/Sound/click.*'),font:new ig.Font('media/04b03.font.png'),init:function(){this.initKeys();this.clearColor='#555';this.compass=document.getElementById('compass').getContext('2d');miner.spawnEntity=ig.game.spawnEntity;ig.music.add('media/Sound/Cave.*');ig.music.volume=0.5;ig.music.play();},initKeys:function(){ig.input.bind(ig.KEY.A,'left');ig.input.bind(ig.KEY.LEFT_ARROW,'left');ig.input.bind(ig.KEY.D,'right');ig.input.bind(ig.KEY.RIGHT_ARROW,'right');ig.input.bind(ig.KEY.DOWN_ARROW,'down');ig.input.bind(ig.KEY.S,'down');ig.input.bind(ig.KEY.W,'up');ig.input.bind(ig.KEY.UP_ARROW,'up');ig.input.bind(ig.KEY.MOUSE1,'leftClick');ig.input.bind(ig.KEY.E,'interact');ig.input.bind(ig.KEY.SPACE,'use');ig.input.bind(ig.KEY.G,'drop');ig.input.bind(ig.KEY.Q,'flashlight');},steerCam:function(){var speed=16;if(ig.input.state('leftCam')){this.screen.x-=speed;}else if(ig.input.state('rightCam')){this.screen.x+=speed;}
if(ig.input.state('downCam')){this.screen.y+=speed;}else if(ig.input.state('upCam')){this.screen.y-=speed;}},update:function(){this.parent();if(!ig.game.player){ig.game.player=ig.game.getEntitiesByType(EntityPlayer)[0];}else{var screenX=this.player.pos.x-ig.system.width/2;var screenY=this.player.pos.y-ig.system.height/2;if(screenX<0){this.screen.x=0;}else if(screenX>(ig.game.collisionMap.data.length-1)*32){this.screen.x=(ig.game.collisionMap.data.length-1)*32;}else{this.screen.x=screenX;}
if(screenY<0){this.screen.y=0;}else if(screenY>(ig.game.collisionMap.data.length-1)*32){this.screen.y=(ig.game.collisionMap.data.length-1)*32;}else{this.screen.y=screenY;}}
if(ig.input.pressed('leftClick')){var tileSize=32,numMapColumns=ig.game.collisionMap.data.length,x=ig.game._rscreen.x+ig.input.mouse.x,y=ig.game._rscreen.y+ig.input.mouse.y,row=Math.floor(x/tileSize),col=Math.floor(y/tileSize),index=(row*numMapColumns)+col;miner.blowUp(col,row);}},updateSurroundingBlocks:function(y,x){var surroundingBlocksSpriteKey=0;var tileAbove=-1;var tileBelow=-1;if(y>0){tileAbove=ig.game.collisionMap.data[y-1][x];}
if(y<(ig.game.collisionMap.data.length-1)){tileBelow=ig.game.collisionMap.data[y+1][x];}
if(tileAbove>0){surroundingBlocksSpriteKey+=1;}
if(ig.game.collisionMap.data[y][x-1]!==undefined&&ig.game.collisionMap.data[y][x-1]>0){surroundingBlocksSpriteKey+=2;}
if(ig.game.collisionMap.data[y][x+1]!==undefined&&ig.game.collisionMap.data[y][x+1]>0){surroundingBlocksSpriteKey+=4;}
if(tileBelow>0){surroundingBlocksSpriteKey+=8;}
if(surroundingBlocksSpriteKey===15){if(ig.game.collisionMap.data[y-1][x-1]>0){surroundingBlocksSpriteKey+=1;}
if(ig.game.collisionMap.data[y-1][x+1]>0){surroundingBlocksSpriteKey+=2;}
if(ig.game.collisionMap.data[y+1][x-1]>0){surroundingBlocksSpriteKey+=4;}
if(ig.game.collisionMap.data[y+1][x+1]>0){surroundingBlocksSpriteKey+=8;}}
if(ig.game.backgroundMaps[0].data[y][x]!==undefined){ig.game.backgroundMaps[0].data[y][x]+=surroundingBlocksSpriteKey;}},drawDarkness:function(){function addLight(ctx,player,allowTiny,offset){var radius=260+(miner.stats().lightrange*200),x,y;if(player&&(player.flashlight||typeof allowTiny!=='undefined')){if(!player.flashlight){radius=40;}
x=(player.pos.x+offset)*2-this.screen.x*2;y=(player.pos.y+offset)*2-this.screen.y*2;ctx.save();ctx.beginPath();var dir=player.direction;var pulse=player.pulse;if(typeof pulse!=='undefined'){pulse=1-pulse;var t=(new Date()).getTime();radius+=Math.sin(pulse*(100+(t/20)))*10;}
if(player.flashlight||allowTiny===true){ctx.arc(x,y,radius,0,2*Math.PI,false);}
ctx.clip();ctx.closePath();ctx.clearRect(0,0,1000,1000);if(typeof pulse!=='undefined'){pulse=(pulse*100)/250;ctx.fillStyle='rgba(200, 30, 30, '+pulse+')';ctx.fillRect(0,0,1000,1000);}
ctx.restore();}}
var ctx=ig.darkness.getContext('2d');ctx.fillStyle="black";ctx.fillRect(0,0,ig.system.canvas.width,ig.system.canvas.height);var players=miner.getPlayers();var offset=0;if(this.player){offset=this.player.size.x/2;}
players.forEach(function(pl){pl.player.flashlight=pl.flashlight;addLight.call(this,ctx,pl.player,false,offset);},this);if(this.player){addLight.call(this,ctx,this.player,true,offset);}
ig.system.canvas.getContext('2d').drawImage(ig.darkness,0,0,ig.system.canvas.width,ig.system.canvas.height);},drawCompass:function(){var ctx=document.getElementById('compass').getContext('2d');ctx.clearRect(0,0,ig.system.canvas.width,ig.system.canvas.height);function drawVector(x,y){var px=512+(x*512);var py=(768/2)+(y*(768/2));var x2=px+((x*-1)*10);var y2=py+((y*-1)*10);var x3=px-((x*-1)*10);var y3=py-((y*-1)*10);ctx.fillStyle='#FFF';ctx.strokeStyle='white';ctx.beginPath();ctx.moveTo(px,py);ctx.lineTo(x2,y2);ctx.lineTo(x3,y3);ctx.stroke();ctx.fill();ctx.closePath();console.log(px+','+py+' '+x2+','+y2+' '+x3+','+y3);}
function magnitude(x,y){return Math.sqrt(x*x+y*y)+0.00001;}
var players=miner.getPlayers();if(this.player){players.forEach(function(player){if(player.type==='player'){var x=(player.player.pos.x+(player.player.size.x/2))-(this.player.pos.x+(this.player.size.x/2));var y=(player.player.pos.y+(player.player.size.x/2))-(this.player.pos.y+(this.player.size.x/2));var mag=magnitude(x,y);x/=mag;y/=mag;drawVector(x,y);}},this);}
ig.system.canvas.getContext('2d').drawImage(document.getElementById('compass'),0,0,ig.system.canvas.width,ig.system.canvas.height);},draw:function(){this.parent();this.drawDarkness();if(this.player){if(miner.hasItem('compass')){this.drawCompass();}
this.player.heart.draw();}}});var resX=512,resY=384,scale=2;ig.main('#canvas',MyGame,60,resX,resY,scale);ig.darkness=document.getElementById('darkness');ig.darkness.width=resX*scale;ig.darkness.height=resY*scale;});